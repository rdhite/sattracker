...
class Settings(BaseSettings):
    ...
    """
    tle_url_tmpl: str = Field(
        "https://celestrak.org/NORAD/elements/gp.php?GROUP={group}&FORMAT=tle",
        description="URL for the TLE data source."
    )
    cache_dir: str = Field("cache", description="Directory to store cached files.")
    tle_cache_file_tmpl: str = Field("{group}.tle", description="Filename for the TLE cache.")
    cache_expiration_hours: int = Field(4, description="Cache validity duration in hours.")
